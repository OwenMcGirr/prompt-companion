...
4. On macOS, click **Paste on next field click**, then click the Codex draft within 30 seconds. After the paste command is posted to the validated field, the unchanged companion draft clears and Undo can restore it. Failures before Cmd+V keep the draft. **Cancel waiting paste** cancels it. On Windows, use **Copy Prompt**, then paste yourself. On macOS, **Copy instead** provides the same fallback; successful copying clears the draft and Undo restores it. Nothing is submitted automatically.
...
The destination must be Codex. A click elsewhere, missing permission, an unsupported/protected field, a changed draft or task, or a 30-second timeout prevents pasting. Editing, clearing, copying, expanding, or changing tasks cancels a waiting paste. A second external click before the attempt cancels it. The app checks the destination's field, selection, surrounding text and focus immediately before posting Cmd+V. It never retries automatically.

Paste replaces plain-text clipboard contents with your draft. Unsupported clipboard formats are left untouched; no timed clipboard restoration is used. Posting both paste key events clears and saves the unchanged companion draft. A newer edit or task switch prevents clearing. Failures before event posting keep the draft. Codex does not reliably expose whether it rendered the posted paste, so Prompt Companion Undo remains available if delivery is blocked. The destination's Undo operates on a delivered paste.
