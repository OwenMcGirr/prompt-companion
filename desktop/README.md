...
macOS also offers **Insert on next field click**: an explicit button arms one mouse-click-triggered capture and insertion, without a return trip to the composer. It expires after 30 seconds, checks the clicked control against the focused editor, and consumes failures without retry. This observes only a temporary external left-click event, not keystrokes. The older capture-first controls remain available.
