...
4. On macOS, click **Paste on next field click**, then click the Codex draft within 30 seconds. Your companion draft stays in place. **Cancel waiting paste** cancels it. On Windows, use **Copy Prompt**, then paste yourself. On macOS, **Copy instead** provides the same fallback; successful copying clears the draft and Undo restores it. Nothing is submitted automatically.
...
Paste replaces plain-text clipboard contents with your draft. Unsupported clipboard formats are left untouched; no timed clipboard restoration is used. Your companion draft remains saved after paste. The destination's Undo operates on the paste; Prompt Companion's Undo operates on its own draft.
