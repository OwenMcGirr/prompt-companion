...
Native Accessibility selected-text replacement returned success without visible text and is not exposed in the normal interface. Clipboard paste is the macOS action. Reacquiring the focused field still produced neither reliable text nor cursor changes after a user-confirmed successful paste. The normal clipboard path therefore clears after both Cmd+V events are posted to the prevalidated field; it does not claim independent delivery confirmation. The unchanged draft/task/revision guard and Undo remain in place.
